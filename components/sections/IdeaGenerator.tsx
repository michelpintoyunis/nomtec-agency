...
const IdeaGenerator: React.FC = () => {
    ...
    const { t } = useLanguage();
    const { ideas, status, generateIdeas } = useIdeaGenerator();

    const handleGenerate = async (e: React.FormEvent) => {
        e.preventDefault();
        await generateIdeas(topic, platform);
    };
            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                <div className="grid lg:grid-cols-2 gap-16 items-start">
                    <div className="text-center md:text-left">
                        ...
                        <h2 className="text-3xl md:text-5xl font-bold mb-6 leading-tight" dangerouslySetInnerHTML={{ __html: t('aiLab.title') }}>
                        </h2>
                        ...
                        <form onSubmit={handleGenerate} className="space-y-6 bg-slate-800/50 p-8 rounded-2xl border border-slate-700 backdrop-blur-sm">
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder={t('aiLab.placeholderTopic')}
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-300 mb-2">{t('aiLab.labelPlatform')}</label>
                            ...
                            >
                                {status === LoadingState.LOADING ? (
                                    <>
                                        <Loader2 className="animate-spin" /> {t('aiLab.btnGenerating')}
                                    </>
                                ) : (
                                    <>
                                        <Sparkles className="w-5 h-5" /> {t('aiLab.btnGenerate')}
                                    </>
                                )}
                            </button>
                        </form>
                    ...
                    <div className="space-y-4">
                        {status === LoadingState.IDLE && (
                            <div className="h-full flex flex-col items-center justify-center text-slate-500 border-2 border-dashed border-slate-700 rounded-2xl p-12 min-h-[400px]">
                                ...
                                <p className="text-center">{t('aiLab.resultPlaceholder')}</p>
                            </div>
                        )}
                        ...
                        {status === LoadingState.SUCCESS && (
                            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                                    <Check className="text-green-400" /> {t('aiLab.resultTitle')} {platform}
                                </h3>
                                {ideas.map((idea, idx) => (
